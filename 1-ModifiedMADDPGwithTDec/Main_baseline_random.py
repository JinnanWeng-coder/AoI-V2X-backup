...
for i_episode in range(n_episode):
    ...
    for i_step in range(n_step_per_episode):
        ...
        for i in range(n_platoon):
            action = np.random.uniform(-0.999, 0.999, size=n_output)
...
print('Baseline random done. Saving results...')
current_dir = os.path.dirname(os.path.realpath(__file__))
save_dir = os.path.join(current_dir, 'model', label, 'baseline_random_seed1')
